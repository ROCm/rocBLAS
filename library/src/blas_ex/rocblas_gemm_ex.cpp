          ...
          | rocblas_layer_mode_log_profile))
    {
        ...
        if(layer_mode & (rocblas_layer_mode_log_bench | rocblas_layer_mode_log_trace))
        {
            if(handle->pointer_mode == rocblas_pointer_mode_host)
            {
                ...
                {
                    alpha_str = rocblas_value_to_string(*static_cast<const _Float16*>(alpha));
                    beta_str  = rocblas_value_to_string(*static_cast<const _Float16*>(beta));
                }
                else if(compute_type == rocblas_datatype_f32_r)
                {
                    alpha_str = rocblas_value_to_string(*static_cast<const float*>(alpha));
                    beta_str  = rocblas_value_to_string(*static_cast<const float*>(beta));
                }
                else if(compute_type == rocblas_datatype_f64_r)
                {
                    alpha_str = rocblas_value_to_string(*static_cast<const double*>(alpha));
                    beta_str  = rocblas_value_to_string(*static_cast<const double*>(beta));
                }
                else if(compute_type == rocblas_datatype_i32_r)
                {
                    alpha_str = rocblas_value_to_string(*static_cast<const int32_t*>(alpha));
                    beta_str  = rocblas_value_to_string(*static_cast<const int32_t*>(beta));
                }
                else if(compute_type == rocblas_datatype_f32_c)
                {
                    alpha_str = rocblas_value_to_string(
                        *static_cast<const rocblas_float_complex*>(alpha));
                    beta_str
                        = rocblas_value_to_string(*static_cast<const rocblas_float_complex*>(beta));
                }
                else if(compute_type == rocblas_datatype_f64_c)
                {
                    alpha_str = rocblas_value_to_string(
                        *static_cast<const rocblas_double_complex*>(alpha));
                    beta_str = rocblas_value_to_string(
                        *static_cast<const rocblas_double_complex*>(beta));
                }
