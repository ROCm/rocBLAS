...
rocblas_status gemm_ex_handle_transpose(rocblas_handle handle,
               ...
               const Td* a, unsigned int lda, unsigned int stride_a,
               const Td* b, unsigned int ldb, unsigned int stride_b, const Tc beta,
               const Td* c, unsigned int ldc, unsigned int stride_c,
                     Td* d, unsigned int ldd, unsigned int stride_d, unsigned int batch_count)
{
    TensileStatus t_status;
    rocblas_status rb_status;

    device_strided_batched_matrix_copy(c, ldc, stride_c, d, ldd, stride_d, m, n, batch_count, sizeof(Td));
...
rocblas_status gemm_ex_chunking(rocblas_handle handle,
                            ...
                            Tc alpha,
                            const Td* a, unsigned int lda, unsigned int stride_a,
                            const Td* b, unsigned int ldb, unsigned int stride_b,
                            Tc beta,
                            const Td* c, unsigned int ldc, unsigned int stride_c,
                            Td* d, unsigned int ldd, unsigned int stride_d,
                            unsigned int batch_count)
{
    ...
    for(int n_chunk_iterator = 0; n_chunk_iterator < n_chunk_count; n_chunk_iterator++)
    {
        ...
        for(int m_chunk_iterator = 0; m_chunk_iterator < m_chunk_count; m_chunk_iterator++)
        {
            ...
            status = gemm_ex_handle_transpose<Td, Tc>(
                            handle,
                            trans_a,
                            trans_b,
                            ...
                            void* d, rocblas_int ldd, rocblas_int stride_d, rocblas_int batch_count)
{
    ...
    return gemm_ex_chunking<Td,Tc>(handle,
                            trans_a,
                            trans_b,
                            static_cast<unsigned int>(m),
                            static_cast<unsigned int>(n),
                            static_cast<unsigned int>(k),
                            h_alpha,
                            static_cast<const Td*>(a), static_cast<unsigned int>(lda), static_cast<unsigned int>(stride_a),
                            static_cast<const Td*>(b), static_cast<unsigned int>(ldb), static_cast<unsigned int>(stride_b),
                            h_beta,
                            static_cast<const Td*>(c), static_cast<unsigned int>(ldc), static_cast<unsigned int>(stride_c),
                            static_cast<      Td*>(d), static_cast<unsigned int>(ldd), static_cast<unsigned int>(stride_d),
                            static_cast<unsigned int>(batch_count));
}
// clang-format on
