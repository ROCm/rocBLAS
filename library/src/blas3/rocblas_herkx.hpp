...
static __device__ void herkx_scale_device(bool upper, rocblas_int n, T beta, U* C, rocblas_int ldc)
{
...
__global__ void herkx_scale_kernel(bool           upper,
                                   rocblas_int    n,
                                   rocblas_int    k,
                                   U              alpha_host_device,
                                   V              beta_host_device,
                                   ...
                                   rocblas_stride stride_c)
{
    ...
    herkx_scale_device(upper, n, beta, C, ldc);
}

template <typename TScal, typename TConstPtr, typename UScal, typename TPtr>
rocblas_status rocblas_herkx_arg_check(rocblas_handle    handle,
                                       rocblas_fill      uplo,
...
/**
  ...
  */
template <typename TScal, typename TConstPtr, typename UScal, typename TPtr>
rocblas_status rocblas_herkx_template(rocblas_handle    handle,
                                      rocblas_fill      uplo,
                                      rocblas_operation trans,
                                      rocblas_int       n,
                                      rocblas_int       k,
                                      ...
                                      rocblas_int       batch_count)
{
    ...
    static constexpr int herkx_SCALE_DIM_X = 128;
    static constexpr int herkx_SCALE_DIM_Y = 8;
    rocblas_int          gx                = (n - 1) / (herkx_SCALE_DIM_X) + 1;
    rocblas_int          gy                = (n - 1) / (herkx_SCALE_DIM_Y) + 1;
    dim3                 herkx_scale_grid(gx, gy, batch_count);
    dim3                 herkx_scale_threads(herkx_SCALE_DIM_X, herkx_SCALE_DIM_Y);
    ...
    if(handle->pointer_mode == rocblas_pointer_mode_device)
    {
        ...
        hipLaunchKernelGGL((herkx_scale_kernel),
                           herkx_scale_grid,
                           herkx_scale_threads,
                           0,
