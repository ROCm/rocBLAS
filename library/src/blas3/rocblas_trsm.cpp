                                     ...
                                     T*                X)
    {
        ...
        if(transA == transB)
        {
            ...
            for( i=0; i < m; i += BLOCK ) {
                jb = min(m-i, BLOCK);
                T *tmp = (i == 0) ? alpha : one;
                rocblas_gemm_template<false, false>(handle, transA, transB, jb, n, jb, tmp, invA, i * BLOCK, BLOCK, 0, B, i, ldb, 0,&zero<T>, X, i, ldb, 0, 1);
                if(i + BLOCK < m){
                    rocblas_gemm_template<false, false>(handle, transA, transB, m-i-BLOCK, n, BLOCK, &negative_one<T>, A,i + BLOCK + i * lda, lda, 0, X, i, ldb, 0, tmp, B, i + BLOCK, ldb, 0, 1);
                }
            }
