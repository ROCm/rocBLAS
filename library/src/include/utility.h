...
// add the offset, if the argument is a T*, add block * stride to pointer and return.

// For device pointers
template <typename T>
__forceinline__ __device__ T* load_ptr_batch(T* p, rocblas_int block, rocblas_int stride)
{
...
template <typename T>
__forceinline__ __device__ T* load_ptr_batch(T** p, rocblas_int block, rocblas_int offset)
{
