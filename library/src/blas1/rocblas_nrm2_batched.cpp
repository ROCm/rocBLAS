...
namespace
{
    ...
    rocblas_status rocblas_nrm2_batched(
        rocblas_handle handle, rocblas_int n, const Ti* const x[], rocblas_int incx, To* results, rocblas_int batch_count)
    {
        if(!handle)
        ...
        if(layer_mode & rocblas_layer_mode_log_profile)
            log_profile(handle, rocblas_nrm2_batched_name<Ti>, "N", n, "incx", incx, "batch", batch_count);
