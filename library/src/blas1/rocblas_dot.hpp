...
#ifndef ROCBLAS_DOT_HPP
#define ROCBLAS_DOT_HPP
...
{
    rocblas_int blocks = (n - 1) / NB_X_DOT + 1;
    ...
    dim3 threads(NB_X_DOT, 1, 1);

    hipStream_t rocblas_stream = handle->rocblas_stream;

    hipLaunchKernelGGL((dot_kernel_part1<T, NB_X_DOT>),
                       dim3(grid),
                       dim3(threads),
                       0,
                       rocblas_stream,
    ...
    if(rocblas_pointer_mode_device == handle->pointer_mode)
    {
        ...
        hipLaunchKernelGGL((dot_kernel_part2<T, NB_X_DOT, 1>),
                           dim3(1, 1, 1),
                           dim3(threads),
                           0,
                           rocblas_stream,
    ...
    else
    {
        ...
        if(blocks > 1)
            hipLaunchKernelGGL((dot_kernel_part2<T, NB_X_DOT, 0>),
                               dim3(1, 1, 1),
                               dim3(threads),
                               0,
                               rocblas_stream,
