...
namespace
{
    ...
    __global__ void rotm_kernel(rocblas_int n, T* x, rocblas_int incx, T* y, rocblas_int incy, U flag_device_host, U h11_device_host, U h21_device_host, U h12_device_host, U h22_device_host)
    {
    ...
    template <class T>
    rocblas_status rocblas_rotm(rocblas_handle handle, rocblas_int n, T* x, rocblas_int incx, T* y, rocblas_int incy, const T* param)
    {
        if (!handle)
        ...
        if (rocblas_pointer_mode_device == handle->pointer_mode)
            hipLaunchKernelGGL(rotm_kernel, blocks, threads, 0, rocblas_stream, n, x, incx, y, incy, param, param + 1, param + 2, param + 3, param + 4);
        else // c and s are on host
            hipLaunchKernelGGL(rotm_kernel, blocks, threads, 0, rocblas_stream, n, x, incx, y, incy, param[0], param[1], param[2], param[3], param[4]);
