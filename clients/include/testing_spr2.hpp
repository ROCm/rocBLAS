...
void testing_spr2_bad_arg()
{
    ...
    EXPECT_ROCBLAS_STATUS(rocblas_spr2<T>(handle, uplo, N, &alpha, dx, incx, nullptr, incy, dA_1),
                          rocblas_status_invalid_pointer);

    EXPECT_ROCBLAS_STATUS(rocblas_spr2<T>(handle, uplo, N, &alpha, dx, incx, dy, incy, nullptr),
                          rocblas_status_invalid_pointer);

    EXPECT_ROCBLAS_STATUS(rocblas_spr2<T>(nullptr, uplo, N, &alpha, dx, incx, dy, incy, dA_1),
...
void testing_spr2(const Arguments& arg)
{
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        CHECK_ROCBLAS_ERROR(rocblas_spr2<T>(handle, uplo, N, &h_alpha, dx, incx, dy, incy, dA_1));

        CHECK_ROCBLAS_ERROR(rocblas_set_pointer_mode(handle, rocblas_pointer_mode_device));
        CHECK_ROCBLAS_ERROR(rocblas_spr2<T>(handle, uplo, N, d_alpha, dx, incx, dy, incy, dA_2));
        ...
        cblas_gflops  = spr2_gflop_count<T>(N) / cpu_time_used * 1e6;
    ...
    if(arg.timing)
    {
        ...
        {
            rocblas_spr2<T>(handle, uplo, N, &h_alpha, dx, incx, dy, incy, dA_1);
        }
