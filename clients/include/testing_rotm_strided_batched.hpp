...
void testing_rotm_strided_batched_bad_arg(const Arguments& arg)
{
    ...
    EXPECT_ROCBLAS_STATUS(
        (rocblas_rotm_strided_batched<T>(
            nullptr, N, dx, incx, stride_x, dy, incy, stride_y, dparam, batch_count)),
        rocblas_status_invalid_handle);
    EXPECT_ROCBLAS_STATUS(
        (rocblas_rotm_strided_batched<T>(
            handle, N, nullptr, incx, stride_x, dy, incy, stride_y, dparam, batch_count)),
        rocblas_status_invalid_pointer);
    EXPECT_ROCBLAS_STATUS(
        (rocblas_rotm_strided_batched<T>(
            handle, N, dx, incx, stride_x, nullptr, incy, stride_y, dparam, batch_count)),
        rocblas_status_invalid_pointer);
    EXPECT_ROCBLAS_STATUS(
        (rocblas_rotm_strided_batched<T>(
            handle, N, dx, incx, stride_x, dy, incy, stride_y, nullptr, batch_count)),
        rocblas_status_invalid_pointer);
}
...
void testing_rotm_strided_batched(const Arguments& arg)
{
        ...
        if(batch_count < 0)
            EXPECT_ROCBLAS_STATUS(
                (rocblas_rotm_strided_batched<
                    T>)(handle, N, dx, incx, stride_x, dy, incy, stride_y, dparam, batch_count),
                rocblas_status_invalid_size);
        else
            CHECK_ROCBLAS_ERROR((rocblas_rotm_strided_batched<T>(
                handle, N, dx, incx, stride_x, dy, incy, stride_y, dparam, batch_count)));
        return;
    }
    ...
    for(int i = 0; i < FLAG_COUNT; i++)
    {
        ...
        {
            cblas_rotm<T>(N, cx + b * stride_x, incx, cy + b * stride_y, incy, hparam);
        }
