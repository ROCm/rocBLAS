...
void testing_hemv_strided_batched_bad_arg(const Arguments& arg)
{
    ...
    EXPECT_ROCBLAS_STATUS(rocblas_hemv_strided_batched<T>(nullptr,
                                                          uplo,
                                                          N,
                                                          &alpha,
                                                          dA,
...
void testing_hemv_strided_batched(const Arguments& arg)
{
    ...
    if(arg.timing)
    {
        ...
        {
            rocblas_hemv_strided_batched<T>(handle,
                                            uplo,
                                            N,
                                            &h_alpha,
                                            dA,
        ...
        rocblas_bandwidth = batch_count * (1.0 * N * N) * sizeof(T) / gpu_time_used / 1e3;

        // only norm_check return an norm error, unit check won't return anything
        std::cout << "N,alpha,lda,stride_A,incx,stride_x,beta,incy,stride_y,batch_count,rocblas-"
                     "Gflops,rocblas-GB/s,";
        if(arg.norm_check)
        {
        ...
        std::cout << N << "," << h_alpha << "," << lda << "," << stride_A << "," << incx << ","
