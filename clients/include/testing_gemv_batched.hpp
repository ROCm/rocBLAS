...
void testing_gemv_batched(const Arguments& arg)
{
    ...
    T** dy_1A;
    T** dy_2A;
    hipMalloc(&dAA, batch_count * sizeof(T*));
    ...
    for(int b = 0; b < batch_count; ++b)
    {
        ...
        hipMalloc(&AA[b], size_A * sizeof(T));
        hipMalloc(&xA[b], size_x * sizeof(T));
        hipMalloc(&y_1A[b], size_y * sizeof(T));
        hipMalloc(&y_2A[b], size_y * sizeof(T));
    }
    ...
    for(int b = 0; b < batch_count; ++b)
    {
        CHECK_HIP_ERROR(hipFree(AA[b]));
        CHECK_HIP_ERROR(hipFree(xA[b]));
        CHECK_HIP_ERROR(hipFree(y_1A[b]));
        CHECK_HIP_ERROR(hipFree(y_2A[b]));
    }
    CHECK_HIP_ERROR(hipFree(dAA));
    CHECK_HIP_ERROR(hipFree(dxA));
    CHECK_HIP_ERROR(hipFree(dy_1A));
    CHECK_HIP_ERROR(hipFree(dy_2A));
}
