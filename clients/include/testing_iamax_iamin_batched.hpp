...
template <typename T>
void template_testing_iamax_iamin_batched_bad_arg(const Arguments& arg, rocblas_iamax_iamin_batched_t<T> func)
{
  rocblas_int
    N         = 100,
    incx      = 1,
...
void template_testing_iamin_iamax_batched(const Arguments& arg, rocblas_iamax_iamin_batched_t<T> func)
{
  ...
  if(batch_count < 0)
    ...
    for(int batch_index = 0; batch_index < batch_count; ++batch_index)
    {
        CHECK_HIP_ERROR(hipMemcpy(hdx[batch_index], hx[batch_index], size_x * sizeof(T), hipMemcpyHostToDevice));
    }
    ...
    if(arg.unit_check || arg.norm_check)
    {
      ...
      CHECK_HIP_ERROR(hipMemcpy(
				rocblas_result_2, d_rocblas_result_2, batch_count * sizeof(rocblas_int), hipMemcpyDeviceToHost));
