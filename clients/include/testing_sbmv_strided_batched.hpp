...
template <typename T>
void testing_sbmv_strided_batched_bad_arg()
{
...
void testing_sbmv_strided_batched(const Arguments& arg)
{
    ...
    if(arg.timing)
    {
        ...
        rocblas_cout << "uplo, N, lda, K, strideA, incx, strideX, incy, stridey, batch_count, "
        ...
        rocblas_cout << arg.uplo << ',' << N << ',' << lda << ',' << K << ',' << strideA << incx
