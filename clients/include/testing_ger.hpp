...
void testing_ger_bad_arg()
{
    ...
    {
        T* dx_null = nullptr;
        status     = rocblas_ger<T>(handle, M, N, &alpha, dx_null, incx, dy, incy, dA_1, lda);
    ...
    {
        T* dy_null = nullptr;
        status     = rocblas_ger<T>(handle, M, N, &alpha, dx, incx, dy_null, incy, dA_1, lda);
    ...
    {
        T* dA_1_null = nullptr;
        status       = rocblas_ger<T>(handle, M, N, &alpha, dx, incx, dy, incy, dA_1_null, lda);
    ...
    // test if (handle == nullptr)
    {
        ...
        status = rocblas_ger<T>(handle_null, M, N, &alpha, dx, incx, dy, incy, dA_1, lda);
...
rocblas_status testing_ger(Arguments argus)
{
    ...
    if(M <= 0 || N <= 0 || lda < M || lda < 1 || 0 == incx || 0 == incy)
    {
        ...
        status = rocblas_ger<T>(handle, M, N, &h_alpha, dx, incx, dy, incy, dA_1, lda);
