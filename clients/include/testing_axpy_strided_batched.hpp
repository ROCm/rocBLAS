...
void testing_axpy_strided_batched_bad_arg(const Arguments& arg)
{
  ...
  rocblas_stride
    stridex     = arg.stride_x,
    stridey     = arg.stride_y;
  ...
  EXPECT_ROCBLAS_STATUS(rocblas_axpy_strided_batched<T>(handle,
							N,
							&alpha,
							nullptr,
							incx,
  ...
  EXPECT_ROCBLAS_STATUS(rocblas_axpy_strided_batched<T>(handle, N, &alpha, dx, incx, stridex, nullptr, incy, stridey, batch_count),
			rocblas_status_invalid_pointer);
  EXPECT_ROCBLAS_STATUS(rocblas_axpy_strided_batched<T>(handle, N, nullptr, dx, incx, stridex, dy, incy, stridey, batch_count),
			rocblas_status_invalid_pointer);
  EXPECT_ROCBLAS_STATUS(rocblas_axpy_strided_batched<T>(nullptr, N, &alpha, dx, incx, stridex, dy, incy, stridey, batch_count),
...
void testing_axpy_strided_batched(const Arguments& arg)
{
  ...
  std::cout << "stridey " <<  stridey << std::endl;
  ...
  if(N <= 0 || batch_count <= 0)
	...
	dy(safe_size);
      ...
      EXPECT_ROCBLAS_STATUS(rocblas_axpy_strided_batched<T>(handle, N, &h_alpha, dx, incx, stridex, dy, incy, stridey, batch_count),
  ...
  // Host memory.
  //
  host_strided_batch_vector<T>
    hx(N,incx,stridex,batch_count),
    hy(N,incy,stridey,batch_count),
    hsolution(N,incy,stridey,batch_count);
  host_vector<T>
    halpha(1);
  

  device_strided_batch_vector<T>
    dx(N,incx,stridex,batch_count),
  ...
  if(arg.unit_check || arg.norm_check)
	...
	// Transfer host to device
	//
	host_strided_batch_vector<T> htmp(N,incy,stridey,batch_count);
	htmp.copy_from(hy);
	...
	// Call routine.
	//
	CHECK_ROCBLAS_ERROR(rocblas_axpy_strided_batched<T>(handle,
							    N,
							    halpha,
							    dx,
							    incx,
  ...
  if(arg.timing)
    {
      ...
      for(int iter = 0; iter < number_cold_calls; iter++)
        {
	  rocblas_axpy_strided_batched<T>(handle, N, &h_alpha, dx, incx, stridex, dy, incy,stridey, batch_count);
        }
      ...
      for(int iter = 0; iter < number_hot_calls; iter++)
	{
      ...
      // Report.
      //
      std::cout << "N,alpha,incx,stridex,incy,stridey,batch,rocblas-Gflops,rocblas-GB/s,rocblas-us";      
      if(arg.norm_check)
	std::cout << "CPU-Gflops,norm_error_host_ptr,norm_error_dev_ptr";      
      std::cout << std::endl;
      std::cout << N << "," << h_alpha << "," << incx << "," << stridex << "," << incy << "," << stridey  << "," << batch_count << "," << rocblas_gflops
