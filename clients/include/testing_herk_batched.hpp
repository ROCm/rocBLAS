...
void testing_herk_batched_bad_arg(const Arguments& arg)
{
    ...
    EXPECT_ROCBLAS_STATUS((rocblas_herk_batched<T, U>)(nullptr,
                                                       uplo,
                                                       transA,
                                                       N,
                                                       K,
...
void testing_herk_batched(const Arguments& arg)
{
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        CHECK_ROCBLAS_ERROR((rocblas_herk_batched<T, U>)(handle,
                                                         uplo,
                                                         transA,
                                                         N,
                                                         K,
        ...
        if(arg.unit_check)
        {
               ...
               || std::is_same<T, rocblas_double_complex>{})
            {
                ...
                near_check_general<T, T>(N, N, batch_count, ldc, hC_gold, hC_1, tol);
                near_check_general<T, T>(N, N, batch_count, ldc, hC_gold, hC_2, tol);
            }
            else
            {
                unit_check_general<T, T>(N, N, batch_count, ldc, hC_gold, hC_1);
                unit_check_general<T, T>(N, N, batch_count, ldc, hC_gold, hC_2);
            }
        }
    ...
    if(arg.timing)
    {
        ...
        {
            rocblas_herk_batched<T, U>(handle,
                                       uplo,
                                       transA,
                                       N,
                                       K,
