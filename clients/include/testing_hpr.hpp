...
void testing_hpr_bad_arg(const Arguments& arg)
{
    ...
    EXPECT_ROCBLAS_STATUS(rocblas_hpr_fn(handle, rocblas_fill_full, N, &alpha, dx, incx, dA_1),
                          rocblas_status_invalid_value);

    EXPECT_ROCBLAS_STATUS(rocblas_hpr_fn(handle, uplo, N, &alpha, nullptr, incx, dA_1),
                          rocblas_status_invalid_pointer);

    EXPECT_ROCBLAS_STATUS(rocblas_hpr_fn(handle, uplo, N, &alpha, dx, incx, nullptr),
                          rocblas_status_invalid_pointer);

    EXPECT_ROCBLAS_STATUS(rocblas_hpr_fn(nullptr, uplo, N, &alpha, dx, incx, dA_1),
...
void testing_hpr(const Arguments& arg)
{
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        CHECK_ROCBLAS_ERROR(rocblas_hpr_fn(handle, uplo, N, &h_alpha, dx, incx, dA_1));

        CHECK_ROCBLAS_ERROR(rocblas_set_pointer_mode(handle, rocblas_pointer_mode_device));
        CHECK_ROCBLAS_ERROR(rocblas_hpr_fn(handle, uplo, N, d_alpha, dx, incx, dA_2));
    ...
    if(arg.timing)
    {
        ...
        {
            rocblas_hpr_fn(handle, uplo, N, &h_alpha, dx, incx, dA_1);
        }
        ...
        rocblas_gflops    = hpr_gflop_count<T>(N) / gpu_time_used * 1e6;
        rocblas_bandwidth = hpr_gbyte_count<T>(N) / gpu_time_used * 1e6;

        // only norm_check return an norm error, unit check won't return anything
        rocblas_cout << "N,alpha,incx,rocblas-Gflops,rocblas-GB/s";
