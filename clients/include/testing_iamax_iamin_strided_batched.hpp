...
void testing_iamax_iamin_strided_batched(const Arguments& arg)
{
  ...
  if(stridex < N * incx)
      ...
      if(!dx)
	{
      ...
      EXPECT_ROCBLAS_STATUS((rocblas_iamax_iamin_strided_batched<T, U>)(handle,
								 N,
								 &h_alpha,
								 dx,
								 incx,
  ...
  if(N <= 0 || incx <= 0
      ...
      if(!dx)
	{
      ...
      CHECK_ROCBLAS_ERROR(rocblas_set_pointer_mode(handle, rocblas_pointer_mode_host));
      CHECK_ROCBLAS_ERROR((rocblas_iamax_iamin_strided_batched<T, U>)(handle,
							       N,
							       &h_alpha,
							       dx,
							       incx,
  ...
  if(arg.unit_check || arg.norm_check)
    {
      ...
      CHECK_ROCBLAS_ERROR(rocblas_set_pointer_mode(handle, rocblas_pointer_mode_host));
      CHECK_ROCBLAS_ERROR((rocblas_iamax_iamin_strided_batched<T, U>(
	...
	{
	  cblas_iamax_iamin<T, U>(N, h_alpha, hx_gold + i * stridex, incx);
	}
  ...
  if(arg.timing)
	...
	{
	  rocblas_iamax_iamin_strided_batched<T, U>(
					     handle, N, &h_alpha, dx_1, incx, stridex, batch_count);
	}
...
void testing_iamax_iamin_strided_batched_bad_arg(const Arguments& arg)
{
  ...
  EXPECT_ROCBLAS_STATUS(
			(rocblas_iamax_iamin_strided_batched<T, U>)(handle, N, nullptr, dx, incx, stridex, batch_count),
