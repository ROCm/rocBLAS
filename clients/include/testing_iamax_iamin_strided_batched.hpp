...
template <typename T>
void template_testing_iamax_iamin_strided_batched_bad_arg(
    const Arguments& arg, rocblas_iamax_iamin_strided_batched_t<T> func)
{
...
void template_testing_iamax_iamin_strided_batched(const Arguments&                         arg,
                                                  rocblas_iamax_iamin_strided_batched_t<T> func)
{
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        CHECK_ROCBLAS_ERROR(rocblas_set_pointer_mode(handle, rocblas_pointer_mode_device));
        CHECK_ROCBLAS_ERROR(
            (func(handle, N, ptrdx, incx, stridex, batch_count, d_rocblas_result_2)));
...
{
    template_testing_iamax_iamin_strided_batched_bad_arg<T>(arg, rocblas_iamax_strided_batched<T>);
}
...
{
    template_testing_iamax_iamin_strided_batched_bad_arg<T>(arg, rocblas_iamin_strided_batched<T>);
}
