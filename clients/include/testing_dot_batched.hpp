...
void testing_dot_batched(const Arguments& arg)
{
    ...
    if(arg.timing)
    {
        static ArgumentModel testing_dot_batched_arg_model(
            std::vector<arg_type>({e_N, e_incx, e_incy, e_batch_count}));
        ...
        testing_dot_batched_arg_model.log_args<T>(std::cout,
                                                  arg,
                                                  gpu_time_used,
