...
void testing_trsv_strided_batched(const Arguments& arg)
{
    ...
    std::cout<<" M "<<M<<" lda "<<lda<<" stride_a "<<stride_a<<" stride_x "<<stride_x<<" incx "<<incx
    <<" batch_count "<<batch_count<<" uplo "<<char_uplo<<" transA "<<char_transA<<" diag "<<char_diag<<std::endl;
    ...
    if(M < 0 || lda < M || !incx || batch_count <= 0)
    {
        ...
        if(batch_count == 0)
            CHECK_ROCBLAS_ERROR(rocblas_trsv_strided_batched<T>(handle, uplo, transA, diag, M, dA, lda, stride_a, dx_or_b, incx, stride_x, batch_count));
        else
            EXPECT_ROCBLAS_STATUS(
                rocblas_trsv_strided_batched<T>(handle, uplo, transA, diag, M, dA, lda, stride_a, dx_or_b, incx, stride_x, batch_count),
    ...
    if(arg.unit_check || arg.norm_check)
    {
        ...
        for(int b = 0; b < batch_count; b++)
        {
            ...
            for(int i = 0; i < M; i++)
            {
                ...
                {
                    std::cout<<"right "<<hx[idx]<<" wrong "<<hx_or_b_1[idx]<<" i "<<i<<" b "<<b<<std::endl;
                    once = true;
                }
                if(hx[idx] != 0)
                {
        ...
        for(int b = 0; b < batch_count; b++)
        {
            cblas_trmv<T>(uplo, transA, diag, M, hA + b * stride_a, lda, hx_or_b_1 + b * stride_x, incx);
            cblas_trmv<T>(uplo, transA, diag, M, hA + b * stride_a, lda, hx_or_b_2 + b * stride_x, incx);
        }
    ...
    if(arg.timing)
    {
        ...
        rocblas_gflops = batch_count * trsv_gflop_count<T>(M) * number_hot_calls / gpu_time_used * 1e6;
        ...
        if(arg.norm_check)
            for(int b = 0; b < batch_count; b++)
                cblas_trsv<T>(uplo, transA, diag, M, hA + b * stride_a, lda, cpu_x_or_b + b * stride_x, incx);
        ...
        std::cout << M << ',' << lda << ',' << incx << ',' << char_uplo << ',' << char_transA << ','
                  << char_diag << ',' << batch_count << ',' << rocblas_gflops << "," << gpu_time_used / number_hot_calls;
