...
def setdefaults(test):
    ...
    if all([x in test for x in ('M', 'incx', 'strideScale')]) and test['function'] in ('ger_strided_batched'):
        test.setdefault('stride_x', int(test['M'] * abs(test['incx']) *
                                    test['strideScale']))
    elif all([x in test for x in ('N', 'incx', 'strideScale')]) and test['function'] in ('dot_strided_batched','dotc_strided_batched'):
        test.setdefault('stride_x', int(test['N'] * abs(test['incx']) *
                                    test['strideScale']))
    else:
       test.setdefault('stride_x', 0)

    if all([x in test for x in ('N', 'incy', 'strideScale')]) and test['function'] in ('ger_strided_batched', 'dot_strided_batched', 'dotc_strided_batched'):
